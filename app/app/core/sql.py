...
def get_db(application):
    try:
        if credentials.get(application):

            connection = mysql.connector.connect(
                host=credentials[application]["DB"].get("HOST"),
                user=credentials[application]["DB"].get("USER"),
                password=credentials[application]["DB"].get("PASSWORD"),
                database=credentials[application]["DB"].get("DB"),
            )
            return connection
    except Error as e:
